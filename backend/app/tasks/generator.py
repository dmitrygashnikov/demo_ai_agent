...
def _build_user_prompt(
    ...
    parts = [
        f"Create a {language} coding task practising the concept '{concept}' "
        f"at difficulty {difficulty} (kind: {kind})."
    ]
    if topic:
        parts.append(
...
def _verify(task: Task) -> tuple[bool, str]:
    """Run the reference solution against ALL tests in the sandbox.

    Returns ``(passed_all, diagnostic)``. Fail-open: an executor error counts as
    a (recoverable) verification failure with the error captured for reflection.
    """
    try:
...
def _parse_candidate(
    ...
    *,
    ...
    return Task(
        id=task_id,
        language=language,
        concept=str(data.get("concept") or concept),
        skill_id=skill_id,
...
def generate_task(
    ...
    *,
    ...
    tests is returned. On exhaustion of the bounded verify/reflect loop (or any
    LLM/search failure) returns ``None`` so the caller falls back to curated
    ...
    messages = [
        {"role": "system", "content": system},
        {
            ...
            "content": _build_user_prompt(
                language, concept, difficulty, topic, grounding, kind
            ),
        },
    ]
            ...
            messages.append(
                {
                    "role": "user",
                    "content": (
                        "Your previous output was missing required keys or had "
                        "empty tests. Return STRICT JSON with non-empty "
                        "visible_tests and hidden_tests and a complete "
                        "reference_solution."
                    ),
                }
            )
